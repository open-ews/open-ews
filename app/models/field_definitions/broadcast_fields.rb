module FieldDefinitions
  BroadcastFields = Collection.new(
    [
      ...
      Field.new(name: "channels", column: Broadcast.arel_table[:channel], schema: FilterSchema::ArrayType.define(type: :string, included_in: Broadcast.channel.values), description: "Must be one of #{Broadcast.channel.values.map { |t| "`#{t}`" }.join(", ")}."),
