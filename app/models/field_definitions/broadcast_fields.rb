module FieldDefinitions
  BroadcastFields = Collection.new(
    [
      Field.new(name: "status", column: Broadcast.arel_table[:status], schema: FilterSchema::ListType.define(type: :string, options: Broadcast::StateMachine.state_definitions.map {  _1.name.to_s }), description: "Must be one of #{Broadcast::StateMachine.state_definitions.map { "`#{_1.name}`" }.join(", ")}."),
