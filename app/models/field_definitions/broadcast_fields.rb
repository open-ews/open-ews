module FieldDefinitions
  BroadcastFields = Collection.new(
    [
      ...
      Field.new(name: "channels", column: Broadcast.arel_table[:channel], schema: FilterSchema::ListType.define(type: :string, options: Broadcast.channel.values), description: "Must be one of #{Broadcast.channel.values.map { |t| "`#{t}`" }.join(", ")}."),
