module FieldDefinitions
  ...
  AlertFields = Collection.new([
    Field.new(name: "status", column: alerts[:status], schema: FilterSchema::ListType.define(:string, Alert::StateMachine.state_definitions.map { _1.name.to_s }), description: "Must be one of #{Alert::StateMachine.state_definitions.map { "`#{_1.name}`" }.join(", ")}."),
