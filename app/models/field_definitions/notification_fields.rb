module FieldDefinitions
  NotificationFields = Collection.new([
    Field.new(name: "status", column: Notification.arel_table[:status], schema: FilterSchema::ListType.define(:string, Notification::StateMachine.state_definitions.map { it.name.to_s }), description: "Must be one of #{Notification::StateMachine.state_definitions.map { "`#{it.name}`" }.join(", ")}."),
