...
RSpec.describe DelayedJob do
  it "re-enqueues the job to run in 15 minutes from now if wait until is longer that 15 minutes" do
    travel_to(Time.zone.local(2018, 1, 1)) do
      ...
      DelayedJob.perform_now(*job_args)
  ...
  it "runs the job immediately when the scheduled_at is in the past" do
    stub_const("DelayedJob::FakeJob", Class.new(ApplicationJob))
    DelayedJob.perform_now("DelayedJob::FakeJob", 1, { foo: "bar", wait_until: 1.second.ago.to_f })

    expect(DelayedJob).not_to have_been_enqueued
    expect(DelayedJob::FakeJob).to have_been_enqueued.with(1, foo: "bar")
  end

  it "only passes through the original job args to the executing job" do
    stub_const("DelayedJob::FakeJob", Class.new(ApplicationJob))
    DelayedJob.perform_now("DelayedJob::FakeJob", 1, nil, 2, wait_until: 1.second.ago.to_f)

    expect(DelayedJob::FakeJob).to have_been_enqueued.with(1, nil, 2)
  end
end
